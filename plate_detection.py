...
class plate():
    ...
    def ratioCheck(self, area, width, height):
        ...
        if (self.type_of_plate == 0):
            aspect = 4.2727
            min = 4000 #30*aspect*30  # minimum area
            max = 30000 #125*aspect*125  # maximum area
    ...
    def cleanPlate(self,plate):
        ...
        self.img_thresh = cv2.adaptiveThreshold(self.img_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
        new,self.contours, hierarchy = cv2.findContours(self.img_thresh.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if self.contours:
